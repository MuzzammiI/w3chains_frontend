...
// Helper function to generate random Gaming NFT data
const generateRandomGamingNFTs = (count) => {
  const nfts = [];
  const nftNames = [
    "Mythic Sword of Aethel", "Cosmic Explorer Spaceship", "Dragon's Scale Armor",
    "Cybernetic Vanguard Skin", "Ancient Tome of Spells", "Starforged Pickaxe",
    "Glacial Griffin Mount", "Phoenix Feather Wand", "Shadow Rogue Daggers",
    "Guardian's Shield", "Arcane Staff of Ages", "Titan's Gauntlets",
    "Legendary Fishing Rod", "Elemental Orb", "Warp-Speed Boots",
    "Crystalline Companion", "Stealth Recon Drone", "Bounty Hunter's Rifle"
  ];
  const games = [
    "Decentraland", "Axie Infinity", "The Sandbox", "Splinterlands",
    "Gods Unchained", "Illuvium", "Alien Worlds", "Gala Games", "Immutable X"
  ];
  const platforms = [
    "Ethereum", "Polygon", "Ronin", "Immutable X", "BNB Chain", "Solana"
  ];
  const genres = [
    "MMORPG", "Strategy", "Adventure", "Card Game", "Battle Royale",
    "Simulation", "RPG", "Collectibles"
  ];
  const rarityLevels = ["Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic"];
  const placeholderColors = [
    '#60A5FA', '#F87171', '#34D399', '#A78BFA', '#FBBF24', '#F472B6',
    '#2DD4BF', '#C084FC', '#FBBF24', '#3B82F6', '#10B981', '#EC4899'
  ...
  for (let i = 0; i < count; i++) {
    ...
    nfts.push({
      ...
      // Unique feature for gaming NFTs: stat boosts
      statBoost: `+${Math.floor(Math.random() * 50) + 10} Attack`,
      owner: `Player${Math.floor(Math.random() * 10000)}`,
    });
  }
  return nfts;
};
...
const GamingNFT = () => {
  ...
  useEffect(() => {
    const generatedNFTs = generateRandomGamingNFTs(100); // Generate a large pool of NFTs
  ...
  useEffect(() => {
    ...
    if (filterGame !== 'All') {
      currentFiltered = currentFiltered.filter(nft => nft.game === filterGame);
    }
  ...
  }, [filterGame, filterPlatform, filterGenre, allNFTs]);
  ...
  const handleCardClick = (id) => {
    ...
    window.location.href = `/gaming-nft-details/${id}`;
  };
  ...
  const uniqueGames = ['All', ...new Set(allNFTs.map(nft => nft.game))];
    ...
    <div className="min-h-screen bg-gray-950 text-white font-inter p-4 overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500 animate-fadeInUp">
            GameFi Universe
          </h1>
          <div className="flex items-center space-x-2 animate-fadeIn">
            <img src="https://placehold.co/30x30/F97316/FFFFFF?text=G" alt="Profile" className="rounded-full border-2 border-orange-400" />
            <span className="text-sm font-semibold hidden sm:inline">My Inventory</span>
          </div>
        </div>

        {/* Featured Top Gaming NFTs Section (Slider) */}
        <div className="bg-gray-800 rounded-xl shadow-lg p-6 mb-12 relative animate-popIn">
          ...
          <div className="relative flex items-center">
            <button
              ...
              aria-label="Previous Featured Gaming NFT"
            >
              ...
              {featuredNFTs.map((nft, index) => (
                ...
                >
                  ...
                  <img
                    ...
                    onError={(e) => { e.target.onerror = null; e.target.src = `https://placehold.co/400x300/444444/FFFFFF?text=FEATURED+NFT`; }}
                  />
                  <div className="p-4">
                    <span className={`inline-block ${getRarityColor(nft.rarity)} text-white text-xs font-semibold px-2 py-0.5 rounded-full mb-1`}>
                      {nft.rarity}
                    </span>
                    <h3 className="text-xl font-bold truncate mb-1">{nft.name}</h3>
                    <p className="text-gray-400 text-sm">Game: {nft.game}</p>
                    <p className="text-green-400 text-lg font-bold mt-2">{nft.price}</p>
                    <p className="text-blue-300 text-sm mt-1">{nft.statBoost}</p> {/* Unique gaming feature */}
                    <button
            ...
            <button
              ...
              aria-label="Next Featured Gaming NFT"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Main Gaming NFT Grid with Filters and Pagination */}
        <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-cyan-500 mb-8 animate-fadeInUp">
          Explore Gaming Assets
        </h2>
        ...
        <div className="flex flex-wrap justify-center sm:justify-start gap-3 mb-8 animate-fadeIn" style={{ animationDelay: '0.7s' }}>
          ...
          >
            ...
            {uniqueGames.map(game => (
              game !== 'All' && <option key={game} value={game}>{game}</option>
            ))}
          </select>
          <select
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 mb-8">
          {paginatedGridNFTs.length > 0 ? (
            paginatedGridNFTs.map((nft, index) => (
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <p className="text-gray-400 text-xs truncate">Game: {nft.game}</p>
                  <p className="text-green-400 text-sm font-bold mt-1">{nft.price}</p>
                  <p className="text-blue-300 text-xs mt-1">{nft.statBoost}</p>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleCardClick(nft.id); }}
            ...
            <div className="col-span-full text-center py-10 text-gray-400 text-lg">
              No Gaming NFTs found matching your criteria.
            </div>
          )}
        </div>
